...
export function CalendarLeftSidebar({
  ...
  return (
    <aside className="w-68 shrink-0 flex flex-col border-r border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/80 backdrop-blur-md p-3.5 space-y-4 overflow-y-auto max-h-[calc(100vh-8.5rem)] select-none">
      {/* Mini Calendário */}
      ...
      {/* Botão de Conectar Novo Colaborador */}
      <button
        type="button"
        onClick={onOpenAddCollaborator}
        className="w-full flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:border-blue-500/50 hover:bg-blue-50/50 dark:hover:bg-blue-950/30 transition cursor-pointer"
      >
        <UserPlus className="w-3.5 h-3.5" />
        <span>+ Conectar Novo Colaborador</span>
      </button>
    </aside>
  );
}
